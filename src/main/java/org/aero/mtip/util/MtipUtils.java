...
public class MtipUtils {
	...
	public static String getUafElementType(Element element) {
		List<Stereotype> stereotypes = StereotypesHelper.getStereotypes(element)
				.stream()
				.filter(x -> UAFProfile.isUafProfile(StereotypesHelper.getProfileForStereotype(x)))
		...
		Logger.log(String.format("UAF type nout found. Ambiguous type for element with id %s. Multiple UAF stereotypes found", element.getID()));
		return null;
	}
