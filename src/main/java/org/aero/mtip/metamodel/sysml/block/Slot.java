...
public class Slot extends CommonElement {
      ...
      if (!values.isEmpty()) {
        ...
        for (String value : values) {
          ...
          ValueSpecification valueSpecification =
              ModelHelper.createValueSpecification(Project.getProject(propertySlot), type, "",
  ...
  public void createDependentElements(HashMap<String, XMLItem> parsedXML, XMLItem modelElement) {
    if (modelElement.hasAttribute(XmlTagConstants.RELATIONSHIP_PROPERTY)) {
      Importer.getInstance().buildElement(parsedXML,
          parsedXML.get(modelElement.getAttribute(XmlTagConstants.RELATIONSHIP_PROPERTY)));
    }

    if (modelElement.hasAttribute(XmlTagConstants.RELATIONSHIP_DEFAULT_VALUE)) {
      Importer.getInstance().buildElement(parsedXML,
          parsedXML.get(modelElement.getAttribute(XmlTagConstants.RELATIONSHIP_DEFAULT_VALUE)));
    }

    if (modelElement.hasAttribute(XmlTagConstants.ATTRIBUTE_KEY_INSTANCE_VALUE)) {
      if (CameoUtils
          .isCameoID(modelElement.getAttribute(XmlTagConstants.ATTRIBUTE_KEY_INSTANCE_VALUE))) {
        Importer.getInstance().buildElement(parsedXML,
            parsedXML.get(modelElement.getAttribute(XmlTagConstants.ATTRIBUTE_KEY_INSTANCE_VALUE)));
      }
    }
  }
